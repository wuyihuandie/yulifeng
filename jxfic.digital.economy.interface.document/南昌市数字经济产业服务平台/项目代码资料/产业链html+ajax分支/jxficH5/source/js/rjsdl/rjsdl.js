...
$(function(){
	...
	$(".classification:eq(1)").click(function(){
		window.location.href=realPath+'view/rjsdl/rjsdl-2.html?menuType=3&CONDITION=2';
	})

	$(".classification:eq(2)").click(function(){
		window.location.href=realPath+'view/rjsdl/rjsdl-2.html?menuType=3&CONDITION=6';
	})
...
//月份新增
function queryJxMonthXzEntDatas() {
	var params = {
		YEAR: YEAR,
		PARENT_DL: PARENT_DL,
		TYPE:'JXMONTH'
			...
			option = {
				tooltip: {
					...
					formatter: function(data) {
						...
						html +='<span style="display:inline-block;margin-right:5px;border-radius:10px;width:10px;height:10px;background-color:#FCB683;"></span>' +
							data[0].seriesName + "：" + data[0].value + title + "<br/>";
...
function renderFZQK() {
	...
	$.ajax({
		...
		success: function(res) {
			...
			let color = ['#E54A49','#E49A50', '#EDD659', '#80DAF5', '#5E75F1', '#5A43F1', '#A246EF',
						'#E249ED','#F91E66', '#F37675', '#FB8A1D']
			...
			var mapData = document.getElementById('banyuan');
			myChart = echarts.init(mapData);
			myChart.setOption(option);
			var canPlayAction = true;
			myChart.on("mouseover", function() {
			...
			timer1 = setInterval(function() {
				...
				var index = seed % seriesData.length;
