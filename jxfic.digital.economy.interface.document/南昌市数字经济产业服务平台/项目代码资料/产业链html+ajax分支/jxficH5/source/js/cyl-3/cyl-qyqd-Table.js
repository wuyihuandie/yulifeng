...
function bindQYList(){
    ...
    doData(BASE_URL+'/Ent03specialController/get03speciaList',params,function(result){
...
function renderQYList(dataList){
    var listHtml='';
	var dataLists =  dataList.Value;
	if(dataLists.length >0){ 
		dataLists.map(function(item,index){
		   var itemHtml=`
		               <tr>
		                   <td class="td1">`+(index+1)+`</td>
		                   <td class="td2" style="width: 20%;">${item.ENTNAME}</td>
		                   <td class="td3">${item.SBDW}</td>
		                   <td class="td4">${item.XMJR}</td>
		                   <td class="td5">${item.AREA_NAME}</td>
		                   <td class="td6">${item.SBLB}</td>
		                   <td class="td7">${item.ENTDATE}</td>
		                   <td class="td8">${item.ISSCRUBBER}</td>
		               </tr>`;
		   listHtml+=itemHtml;
		})
